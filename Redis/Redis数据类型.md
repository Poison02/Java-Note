...
<a name="H4F0Q"></a>
## List
在Redis里面，能将List当成栈、队列、阻塞队列<br />所有的List命令都是以l开头的。
```shell
...
<a name="rU43V"></a>
## Hyperloglog
什么是基数？不重复的元素，接收误差<br />简介：基数统计的算法。<br />优点：占用内存是固定的，2^64不同的元素，只需要12KB内存！
```shell
...
<a name="Yjs6l"></a>
## Bitmaps
位存储<br />统计用户信息，活跃，不活跃。登录、未登录...<br />使用二进制来记录<br />示例：记录周一到周日的打卡记录，0代表打卡，反之。
```shell
