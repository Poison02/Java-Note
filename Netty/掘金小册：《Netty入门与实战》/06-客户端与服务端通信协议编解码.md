...
![image.png](./assets/1653028b3b0a2ef0tplv-t2oaga2asx-zoom-in-crop-mark3024000.webp)
...
![image.png](./assets/1653028b3e5c1437tplv-t2oaga2asx-zoom-in-crop-mark3024000.webp)
...
在本小册子的[第一节](https://juejin.cn/book/6844733738119593991/section/6844733738224451591)中，我们已经列出了实现一个支持单聊和群聊的 IM 的指令集合，我们设计协议的目的就是为了客户端与服务端能够识别出这些具体的指令。 接下来，我们就来看一下如何设计这个通信协议。

## 通信协议的设计

![image.png](./assets/1653028b36ee5d81tplv-t2oaga2asx-zoom-in-crop-mark3024000.webp)
...
2. `@Data` 注解由 [lombok](https://link.juejin.cn/?target=https%3A%2F%2Fwww.projectlombok.org%2F) 提供，它会自动帮我们生产 getter/setter 方法，减少大量重复代码，推荐使用。
...
```java
java复制代码public interface Serializer {
...
序列化接口有三个方法，`getSerializerAlgorithm()` 获取具体的序列化算法标识，`serialize()` 将 Java 对象转换成字节数组，`deserialize()` 将字节数组转换成某种类型的 Java 对象，在本小册中，我们使用最简单的 json 序列化方式，使用阿里巴巴的 [fastjson](https://link.juejin.cn/?target=https%3A%2F%2Fgithub.com%2Falibaba%2Ffastjson) 作为序列化框架。

```java
java复制代码public interface SerializerAlgorithm {
    /**
     * json 序列化标识
     */
    byte JSON = 1;
...
```java
java复制代码private static final int MAGIC_NUMBER = 0x12345678;
...
```java
java复制代码public Packet decode(ByteBuf byteBuf) {
    // 跳过 magic number
    byteBuf.skipBytes(4);
...
> 本小节完整代码参考 [github](https://link.juejin.cn/?target=https%3A%2F%2Fgithub.com%2FlightningMan%2Fflash-netty%2Ftree%2F%E5%AE%A2%E6%88%B7%E7%AB%AF%E4%B8%8E%E6%9C%8D%E5%8A%A1%E7%AB%AF%E9%80%9A%E4%BF%A1%E5%8D%8F%E8%AE%AE%E7%BC%96%E8%A7%A3%E7%A0%81)， 其中 `PacketCodeCTest.java` 对编解码过程的测试用例，大家可以下载下来跑一跑。
